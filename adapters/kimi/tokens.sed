s#{{KIT_DISPLAY_NAME}}#Kimi Agent#g
s#{{HARNESS_NAME}}#Kimi Code CLI#g
s#{{HARNESS_HOME}}#$KIMI_CODE_HOME#g
s#{{PRIMARY_MANUAL_FILE}}#AGENTS.md#g
s#{{TASK_EXECUTION_RULE_FILE}}#kimi-agent-kit--task-execution.md#g
s#{{PALETTE_RULE_FILE}}#kimi-agent-kit--palette.md#g
s#{{DELEGATION_RULE_FILE}}#kimi-agent-kit--delegation.md#g
s#{{GIT_WORKFLOW_RULE_FILE}}#kimi-agent-kit--git-workflow.md#g
s#{{FRAMEWORK_RULE_FILE}}#kimi-agent-kit--framework-conventions.md#g
s#{{ASIDE_RULE_FILE}}#kimi-agent-kit--aside.md#g
s#{{DISPATCH_RULE_FILE}}#kimi-agent-kit--dispatch.md#g
s#{{ASIDE_PREFS_FILE}}#kimi-agent-kit--aside-prefs.md#g
s#{{DISPATCH_PREFS_FILE}}#kimi-agent-kit--dispatch-prefs.md#g
s#{{GIT_PREFS_FILE}}#kimi-agent-kit--git-prefs.md#g
s#{{COMMENT_PREFS_FILE}}#kimi-agent-kit--comment-prefs.md#g
s#{{HARNESS_RULES_DIR}}#$KIMI_CODE_HOME/rules#g
s#{{EDIT_SURFACE}}#Kimi's native edit/write tools#g
s#{{DELEGATION_SURFACE}}#Agent / dispatch when installed#g
s#{{FANOUT_DELEGATION_SURFACE}}#AgentSwarm#g
s#{{KIT_PREFIX}}#kimi-agent-kit#g
s#{{KIT_VERSION}}#0.7.1#g
