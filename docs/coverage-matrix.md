...
| claude `delegation-surfaces` insert: Agent Teams composition, coordination, leader workflow, teammate behavior, completion report, workslate messaging, Workflow quality flow, `model="sonnet"` default | removed. Kept: which surfaces are read-only vs write-capable, the Workflow opt-in rule, hold an interrupted delegate. Changed: delegates may call the native advisor; aside and dispatch from a delegate need the user's explicit approval (now in loop-delegation.md for every harness) | superseded |
...
| loop-execution "File headers" (12.0.0): header presence, structured layout | presence, layout, comment language, and doc comment coverage moved into the user-owned `comment-prefs` file, ranked below the repository's own convention. The rule keeps the precedence order, what a header leaves out, the keep-the-format rule for an edited header, and the session-context rule. "Who calls it" was dropped from the layout | superseded |
