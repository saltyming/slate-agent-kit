...
**File headers and comment style:** whether a file carries a header, the header's layout, the language of comments, and how far public items are documented follow the repository's own convention. Where the repository leaves one open, they follow `{{HARNESS_RULES_DIR}}/{{COMMENT_PREFS_FILE}}`; read it once per session, before you create a file or write a comment, doc comment, or header, including in a file you are only editing. A current-turn instruction from the user outranks both, and a line under that file's "Repository overrides" outranks the repository's convention. Whatever the layout, a header states what the file is responsible for, and it leaves out references to a plan, phase, or ticket, design deliberation, and future intentions. When you edit an existing header, keep its format and change only what your edit made untrue. Write every comment and header for a reader who has none of your session context: a phrase that is clear only inside the work you just did ("the existing lifecycle driver") does not belong there. Reason: the first lines of a file are what the next reader, human or agent, uses to decide whether to read the rest.
