...
//!
//! Duplicated verbatim in mcp-servers/aside/src/errkind.rs, matching this
...
mod tests {
    ...
    fn recognizes_real_captured_claude_bad_model_shape() {
        ...
        // the exact string `failure_text` builds (stderr + stdout), so it also
        // witnesses that the failure path must forward stdout — without it this
        // classifies as `Other` and model_fallback never advances.
